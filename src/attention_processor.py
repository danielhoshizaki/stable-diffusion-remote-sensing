...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        query = attn.head_to_batch_dim(query).contiguous().to(torch.bfloat16)
        key = attn.head_to_batch_dim(key).contiguous().to(torch.bfloat16)
        value = attn.head_to_batch_dim(value).contiguous().to(torch.bfloat16)
